 ...
 * Muestra toda la información de la solicitud junto con el desglose
 * completo de la cotización SISETAC si existe.
 */
...
export default async function SolicitudDetallePage(
  ...
  if (cotizacion && solicitud.destino) {
    const raw = await consultarRndc(solicitud.origen, solicitud.destino, pesoKg)
    if (raw) {
      rndcData = {
        ...raw,
      ...
      <main style={{ background: '#F3F4F6', minHeight: '100vh', padding: '32px 16px 64px' }}>
        <div style={{ maxWidth: 960, margin: '0 auto' }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 24, fontSize: 13, color: '#6B7280' }}>
            ...
            <span>›</span>
            <span style={{ color: '#374151' }}>Solicitud {id.slice(-6).toUpperCase()}</span>
          </div>
          ...
          <div style={{
            ...
            <div style={{ textAlign: 'right', fontSize: 12, color: '#9CA3AF' }}>
              <div>ID: <code style={{ fontSize: 11, background: '#F3F4F6', padding: '2px 6px', borderRadius: 4 }}>{id}</code></div>
